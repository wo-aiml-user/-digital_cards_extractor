...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const single = payload?.cardData as CardData | undefined;

    if ((!cards || cards.length === 0) && !single) {
    ...
    // --- Build JWT manually for service account ---
    ...
    const allCards = cards.length > 0 ? cards : [single!];
    const rows = allCards.map((d) => [
      d?.name || "",
      d?.company || "",
      d?.job_title || "",
      d?.email || "",
    ...
    // --- Append rows (VERTICAL, anchored to A1) ---
    const appendURL = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/cards_details!A1:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`;
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `${rows.length} row${
          rows.length > 1 ? "s" : ""
        } appended to Google Sheets (cards_details!A1)`,
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      }
